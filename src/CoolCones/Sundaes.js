    ...
    return (
        <>
            <div className="container mt-4" style={{ border: '1px solid #151617' }}>
                <div className="row">
                    {Sundaes.map(({ name, id, image, price, weight, incart}, index) => (
                            ...
                            <Link to='/menu/itemdetails'>  <img onClick={() => Sitem(index)} src={image} height="200px"  class="card-img-top " /></Link>
                                ...
                                <Link to='/menu/itemdetails'>  <h5 className="card-title " onClick={() => Sitem(index)}>{name}</h5></Link>
                                    <div className="d-flex justify-content-between">
                                        ...
                                        <button disabled={incart=="true"?true:false} className="btn btn-primary btn-sm" onClick={() => addSitem(index)}>BuyNow</button>
                                   </div>
                            </div>
                        </div>
                    ))}
